...
# When SO101_URDF_FILE is set (Docker), use the pre-generated URDF file via `cat`
# instead of running xacro at runtime. This sidesteps subprocess environment issues.
_SO101_URDF_FILE = os.environ.get("SO101_URDF_FILE", "")
...
def generate_launch_description():
    ...
    if _SO101_URDF_FILE and os.path.isfile(_SO101_URDF_FILE):
        # Docker: use pre-generated URDF (cat is reliable; xacro subprocess can fail
        # silently in container environments due to environment/shell differences)
        robot_description = ParameterValue(
            Command(["cat ", _SO101_URDF_FILE]),
            value_type=str,
        )
    else:
        robot_description = ParameterValue(
